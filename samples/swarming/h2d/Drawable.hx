...
class DrawableShader extends h3d.impl.Shader {
	...
	static var SRC = {
		...
		*/
		...
		var hasPaletteMap	: Bool = false;
		var paletteMap		: Texture;
		...
		function fragment( tex : Texture ) {
			...
			if( !hasFXAA ){
				var frag = tex.get(sinusDeform != null ? [tcoord.x + sin(tcoord.y * sinusDeform.y + sinusDeform.x) * sinusDeform.z, tcoord.y] : tcoord, filter = ! !filter, wrap = tileWrap);
				if ( hasPaletteMap ) {
					col.xyz = paletteMap.get([frag.x + 0.5 / 255.0, 0.5 / 255.0], filter = ! !filter).xyz;
					col.a = frag.a;
				}
				else 
					col = frag;
			...
			if( isAlphaPremul ) 		col.rgb *= col.a + 0.000001;
	...
	public var hasPaletteMap(default,set) : Bool;	    public function set_hasPaletteMap(v)		{ if( hasPaletteMap != v ) 	invalidate();  	return hasPaletteMap = v; }
	...
	override function getConstants( vertex : Bool ) {
		...
		if( hasPaletteMap )		cst.push("#define hasPaletteMap");
...
class Drawable extends Sprite {
	...
	*/
	...
	#if flash
	public var paletteMap(default, set) : h2d.Tile;
	#end
	...
	*/
	...
	#if flash
	function set_paletteMap(t:h2d.Tile) {
		if( (t != null && paletteMap == null) 
		||	(t == null && paletteMap != null) )
			shader.invalidate();
		
		paletteMap = t;
		shader.hasPaletteMap = t != null;
		shader.paletteMap = t==null?null:t.getTexture();
