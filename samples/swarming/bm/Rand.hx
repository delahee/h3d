...
class Rand {
	...
	@:noDebug
	public inline function percent( val : Int ) : Bool {
		if (val == 0 ) return false;
		return dice(1, 100) <= val;
	}
	
	@:noDebug
	public inline function percentF( val : Float ) : Bool {
		if ( val >= 100.0) return true;
		return diceF(0, 99.999) <= val;
	}
	
	@:noDebug
	public inline function interval( val : Float ) : Float {
		return diceF(-val, val);
	}
	...
	public static inline function fromArray( arr:Array<Int> ) {
		...
		for( i in 0...arr.length){
			var r = new bm.Rand(arr[i]+curSeed);
			curSeed += r.random(1024 * 1024 * 1024);
		}
		return new bm.Rand(curSeed);
	}
}
