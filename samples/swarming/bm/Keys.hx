...
class Keys {
	...
	var prevFrameKeys : haxe.ds.Vector < Bool > = new haxe.ds.Vector( 256 );
	var frameKeys : haxe.ds.Vector < Bool > = new haxe.ds.Vector( 256 );
	var keyDown : haxe.ds.Vector < Bool > = new haxe.ds.Vector( 256 );
	
	public var customOnKeyDown : flash.events.KeyboardEvent -> Void;
	
	public function new() {
		for( i in 0...256 ){
			prevFrameKeys[i] = false;
			frameKeys[i] = false;
			keyDown[i] = false;
		}
	}
	...
	public function update() {
		frameKeys.blit( 0, prevFrameKeys, 0, 256 );
		keyDown.blit( 0, frameKeys, 0, 256 );
	}
	...
	public inline function onPress( kc : Int ) {
		return frameKeys[kc]&&!prevFrameKeys[kc];
	}
	...
	public function keycodeToCode(kc) : String {
		return
		switch(kc) {
			default				: "<error>";			
