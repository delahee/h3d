...
class FrameData {
	...
	public var wid			: Int;
	public var hei			: Int;
	public var realFrame	: { x:Int, y:Int, realWid:Int, realHei:Int };
	...
	public static inline function fromAnon(t:{x:Int,y:Int,wid:Int,hei:Int, realFrame:{ x:Int, y:Int, realWid:Int, realHei:Int }, rect:flash.geom.Rectangle,?pX:Float,?pY:Float }) {
		var f = new FrameData();
		f.x				= t.x		;
		f.y		    	= t.y		;
		f.wid		    = t.wid		;
		f.hei		    = t.hei		;
		f.realFrame   	= t.realFrame;
		f.rect			= t.rect	;
		f.pX		    = t.pX		;
		f.pY		    = t.pY		;
		return f;
...
class BLib {
	 ...
	 */
	var groups					: mt.gx.StringMap<LibGroup>;
	...
	public inline function hbe_get(sb:SpriteBatch, k:String, ?frame=0, ?xr=0., ?yr=0.) : HSpriteBE {
		...
		e.a.setGeneralSpeed( defaultSpeed );
		return e;
	}
	
	public inline function hmbe_get(sb:h2d.MultiSpriteBatch, k:String, ?frame=0, ?xr=0., ?yr=0.) : HSpriteMBE {
		var e = new HSpriteMBE(sb, this, k, frame);
		e.setCenterRatio(xr, yr);
		e.a.setGeneralSpeed( defaultSpeed );
		return e;
	}
	...
	public inline function hmbe_getAndPlay(sb:MultiSpriteBatch, k:String, ?plays=99999, ?killAfterPlay=false) : HSpriteMBE {
		var s = hmbe_get(sb, k);
		s.a.play(k, plays);
		if ( killAfterPlay ) s.a.killAfterPlay();
		return s;
	}
	#end
	...
	public inline function hasTile( g:String  ) : Bool 				return getFrameData(g) != null;
	...
	public inline function getGroups() : mt.gx.StringMap<LibGroup> {
		return groups;
	}
	...
	public function sliceCustom(groupName:String, frame:Int, x:Int, y:Int, wid:Int, hei:Int, ?realFrame:{x:Int, y:Int, realWid:Int, realHei:Int}, ?pX:Float,?pY:Float) {
		...
		var f = g.frames[frame] = new FrameData();
		
		f.x=x;
		f.y=y;
		f.wid=wid;
		f.hei=hei;
		f.realFrame=realFrame;
		f.rect=new Rectangle(x, y, wid, hei);
		f.pX=pX;
		f.pY=pY;
	}
	...
	public function slice(groupName:String, x:Int, y:Int, wid:Int, hei:Int, ?repeatX=1, ?repeatY=1) {
		...
		for(iy in 0...repeatY)
			for(ix in 0...repeatX)
				g.frames.push( FrameData.fromAnon({ x : x + ix * wid, y : y + iy * hei, wid:wid, hei:hei, realFrame: { x:0, y:0, realWid:wid, realHei:hei }, rect:new Rectangle(x + ix * wid, y + iy * hei, wid, hei) }));
	}
	...
	public function sliceGrid(groupName:String, gx:Int, gy:Int, ?repeatX=1, ?repeatY=1) {
		...
		for(iy in 0...repeatY)
			for(ix in 0...repeatX)
				g.frames.push( FrameData.fromAnon({ x : gridX*(gx+ix), y : gridY*(gy+iy), wid:gridX, hei:gridY, realFrame:{x:0,y:0,realWid:gridX,realHei:gridY}, rect:new Rectangle(gridX*(gx+ix), gridY*(gy+iy), gridX, gridY) }));
	}
