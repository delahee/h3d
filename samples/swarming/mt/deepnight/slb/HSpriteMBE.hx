...
class HSpriteMBE extends MultiBatchElement implements SpriteInterface {
	...
	public function new(sb:MultiSpriteBatch, l:BLib, g:String, ?f=0) {
		super( l.tile.clone() );
		destroyed = false;
	...
	public function reset() {
		...
		setPos(0, 0);
		rotation = 0;
		color.setColor( 0xffffff , 1);
		visible = true;
	}

	public function toString() return "HSpriteMBE_"+groupName+"["+frame+"]";
	...
	/**
	 * retrieves the real untrimmed width
	 */
	public inline function frWidth()  return frameData.realFrame.realWid;
	public inline function frHeight()  return frameData.realFrame.realHei;
	...
	function updateTile() {
		...
		if ( flippedX ) tile.targetFlipX();
		if ( flippedY ) tile.targetFlipY();
		
		onTileSync(tile);
	}
	
	public dynamic function onTileSync(tile) { };
	...
	public inline function asCharacter() {
